...
public struct ChartResponse: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let chartContainer = try? container.nestedContainer(keyedBy: ChartKeys.self, forKey: .chart) {
            self.data = try? chartContainer.decodeIfPresent([ChartData].self, forKey: .result)
            self.error = try? chartContainer.decodeIfPresent(ErrorResponse.self, forKey: .error)
        } else {
            self.data = nil
            self.error = nil
        }
