...
public struct QuoteResponse: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        {
            self.data = try?
                financeResponseContainer.decodeIfPresent([Quote].self, forKey: .result)
            self.error = try?
...
public struct Quote: Codable, Identifiable, Hashable {
    ...
    public init(
        ...
        regularMarketChangePreviousClose: Double?,
        postMarketPrice: Double?,
        postMarketPriceChange: Double?,
        ...
        averageDailyVolume3Month: Double?,
